...
public class IbusClient implements ServletContextListener{
	...
	private ZBUSWatched watched = ZBUSWatched.getInstance();
	...
	public void getTopicMsg()throws Exception{
		...
		//2) 创建消费者 
		Consumer c = new Consumer(broker, PropertiesUtil.getKeyValue("IBUS_TOPIC"), MqMode.PubSub); 
		c.setTopic(PropertiesUtil.getKeyValue("IBUS_TOPIC").trim()); 
	...
	public void getQueueMsg()throws Exception{
		...
		final Broker broker = new SingleBroker(config);
		//2) 创建消费者 
		Consumer c = new Consumer(broker, PropertiesUtil.getKeyValue("EBUS_QUEUE"), MqMode.MQ); 
		final AtomicLong counter = new AtomicLong(0);
		c.onMessage(new MessageHandler() { 
			@Override
			public void handle(Message msg, Session sess) throws IOException {
